        ...
        getLatestKeys username, (err, keys) ->
          ...
          validateUser username, password, authSig,(err, status, user) ->
            ...
            rc.del "kt:#{username}", (err, rdel) ->
              ...
              #if we have a gcm Id, set it as the only one as other devices shouldn't be receiving push messages when they have old keys

              gcmId = req.body.gcmId
              if gcmId?
                logger.debug "setting gcmid for #{username}"
                ...
                version = req.body.version
                if version?
                  logger.debug "keys regenerated by client that knows to send version (#{version}), removing gcmid from #{username}"
