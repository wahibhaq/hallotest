...
rc.keys "cu:*", (err, ms) ->
  ...
  console.log "migrating potential #{ms.length} sets of control messages"
  count = 0
  setcount = 0
  async.each(
    ms
    (counterkey, callback) ->
      ...
      if splits.length isnt 2
        console.log "skipping #{counterkey}"
        callback()
      else
        console.log "migrating #{counterkey}, setcount: #{++setcount}"


        rc.zrange counterkey, 0,  -1, (err, messages) ->
          if err?
            console.log "error getting user control messages for #{counterkey}, err: #{err}"
            process.exit(10)
          console.log "migrating #{messages.length} ucmessages from set #{counterkey}"
          ...
          async.each(
            ...
            (err) ->
              ...
              #              console.log "deleting messages #{counterkey}"
    ...
    (err) ->
      ...
      console.log "migrated #{count} setcount: #{setcount}"
