...
rc.keys "d:*", (err, ms) ->
  ...
  console.log "migrating #{ms.length} sets of messages"
  count = 0
  messageCount = 0
  async.each(
    ms
    (counterkey, callback) ->
      ...
      if splits.length isnt 4
        console.log "skipping #{counterkey}"
        callback()
      else
        console.log "migrating #{counterkey}"

        rc.smembers counterkey, (err, messages) ->
          if err?
            console.log "error getting messages for #{counterkey}, err: #{err}"
            process.exit(10)
          console.log "migrating #{messages.length} messages #{messageCount+=messages.length} from set #{counterkey} #{count++}"
          cdb.migrateDeleteMessages splits[1], "#{splits[2]}:#{splits[3]}", messages, (err, results) ->
            if err?
